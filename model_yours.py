...
class HeadProj_DGCNN(nn.Module):
    def __init__(self, cfg, output_dims=40):
        super(HeadProj_DGCNN, self).__init__()
        self.cfg = cfg
        self.output_dims = output_dims


        self.linear1 = nn.Linear(self.cfg.encoder_dims * 2, 512, bias=False)
        self.bn6 = nn.BatchNorm1d(512)
        self.dp1 = nn.Dropout(p=self.cfg.dropout)
        self.linear2 = nn.Linear(512, 256)
        self.bn7 = nn.BatchNorm1d(256)
        self.dp2 = nn.Dropout(p=self.cfg.dropout)
        self.linear3 = nn.Linear(256, self.output_dims)
...
class MAE3D_cls(nn.Module):
    def __init__(self, cfg, encoder_dims=1024):
        super(MAE3D_cls, self).__init__()
        self.cfg = cfg
        self.patch_embed = PatchEmbed_DGCNN(output_channels=cfg.encoder_dims)
        self.head_projection = HeadProj_DGCNN(cfg)
...
class MAE3D(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.masking_strategy = cfg.masking_strategy
        self.patch_num = self.cfg.num_points // self.cfg.patch_size
        ...
        self.token_init = nn.Parameter(torch.randn(self.cfg.decoder_dims, requires_grad=True))
        self.cls_token_encoder = nn.Parameter(torch.randn(1, 1, self.cfg.encoder_dims))
        self.cls_token_decoder = nn.Parameter(torch.randn(1, 1, self.cfg.decoder_dims))

        self.pos_embed_encoder = nn.Sequential(
            nn.Linear(3, self.cfg.encoder_dims),
            nn.ReLU(),
            nn.Linear(self.cfg.encoder_dims, self.cfg.encoder_dims)
        )

        self.pos_embed_decoder = nn.Sequential(
            nn.Linear(3, self.cfg.decoder_dims),
            nn.ReLU(),
            nn.Linear(self.cfg.decoder_dims, self.cfg.decoder_dims)
        )

        self.encoder_to_decoder = nn.Linear(self.cfg.encoder_dims, self.cfg.decoder_dims, bias=False)


        # Point Cloud Reconstruction
        self.num_patch = self.cfg.patch_size
        self.num_center = int(self.cfg.num_points / self.num_patch)

        self.mlp_center = nn.Sequential(
            nn.Linear(self.cfg.decoder_dims, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 1024),
            nn.BatchNorm1d(1024),
    ...
    def forward(self, x):
        ...
        if self.masking_strategy == 'random':
            mask_pos, vis_pos, mask_center_pos, vis_center_pos, mask_patch_idx, vis_patch_idx, shuffle_idx = split_knn_patches(
                xyz, mask_ratio=self.cfg.mask_ratio, nsample=self.cfg.patch_size, random=True)
        ...
        vis_pos_input = vis_pos.view(batch_size, self.vis_patch_num * self.cfg.patch_size, 3)

        x_pos_embed_encoder = self.pos_embed_encoder(center_pos)
        x_pos_embed_decoder = self.pos_embed_decoder(center_pos)
        x_pos_embed_encoder = x_pos_embed_encoder[batch_idx, vis_patch_idx].view(batch_size, -1, self.cfg.encoder_dims)
        x_pos_embed_encoder = torch.cat((self.cls_token_encoder.repeat(batch_size, 1, 1), x_pos_embed_encoder), dim=1)
        x_pos_embed_decoder = x_pos_embed_decoder.view(batch_size, -1, self.cfg.decoder_dims)
...
class MAE3DEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        # MAE3D Transformers
        self.cls_token = nn.Parameter(torch.randn(1, 1, self.cfg.encoder_dims))
        self.transformer = Transformer(
            self.cfg.encoder_dims, self.cfg.mlp_dim, depth=self.cfg.depth, num_heads=self.cfg.num_heads,
            dim_per_head=self.cfg.dim_per_head, dropout=self.cfg.dropout
        )
    ...
    def forward(self, x, x_pos_embed):
        ...
        x = rearrange(x, 'b (p n) c -> (b p) n c', b=batch_size, n=self.cfg.patch_size, c=self.cfg.encoder_dims)
        x = torch.max(x, 1)[0]
        x = x.view(batch_size, -1, self.cfg.encoder_dims)
...
class MAE3DDecoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.decoder_dims = self.cfg.decoder_dims
        self.patch_num = self.cfg.num_points // self.cfg.patch_size
        self.mask_patch_num = int(self.patch_num * self.cfg.mask_ratio)
        self.vis_patch_num = self.patch_num - self.mask_patch_num

        self.cls_token = nn.Parameter(torch.randn(1, 1, self.decoder_dims))
        self.transformer = Transformer(
            self.decoder_dims, self.cfg.mlp_dim, depth=self.cfg.depth, num_heads=self.cfg.num_heads,
            dim_per_head=self.cfg.dim_per_head, dropout=self.cfg.dropout
        )
...
@hydra.main(config_path='config/experiments', config_name='DEBUG', version_base=None)
