...
from util_yours import cal_loss_cd, write_plyfile, stepwise_scheduler
...
def training(cfg, logger): 
    ...
    if cfg.model.masking_strategy == 'frequency':
        cutoff_scheduler = stepwise_scheduler(
            cfg.model.cutoff_schedule,
            default=cfg.model.default_cutoff
        )
    ...
    for epoch in range(start_epoch, cfg.pretraining.epochs):
        ...
        for batch, (data, index) in enumerate(tqdm(train_loader)):
            ...
            # Use cutoff ratio for frequency masking if specified
            if cfg.model.masking_strategy == 'frequency':
                cutoff_ratio = cutoff_scheduler(epoch)
                pred_pc, pred_center, gt_center, vis_pos, crop_pos = model(data, cutoff_ratio=cutoff_ratio)
                wandb.log({'cutoff_ratio': cutoff_ratio}) # log cutoff_ratio 
            else:
